...
statemodel <- lm(energy ~ metro, data = states.data)
summary(statemodel)
plot(statemodel)

stateincomemodel <- lm(energy ~ metro + income, data = na.omit(states.data))
...
stateincomepct <- lm(energy ~ metro*income, data = states.data)
coef(summary(stateincomepct))

stateregion2 <- lm(energy ~ region, data = states.data)
coef(summary(stateregion2))
anova(stateregion2)
contrasts(states.data$region)
coef(summary(lm(energy ~ C(region, base = 4), data = states.data)))
coef(summary(lm(energy ~ C(region, contr.helmert), data = states.data)))

#Region is not sig across four regions